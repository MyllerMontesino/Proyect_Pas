...
function mostrarHistorial() {
  ...
  getDoc(doc(dbHistorial, "DataPas", "Historial")).then((docSnapshot) => {
    if (docSnapshot.exists()) {
      ...
      led2.forEach((fecha) => {
        registros.push({ tipo: 'Cambio Estado Ventiladores', fecha: fecha });
      });
...
document.getElementById("programarBtn1luces").addEventListener("click", () => {
  Swal.fire({
    ...
    preConfirm: () => {
      ...
      if (dateTimeValue) {
        ...
        const label = document.getElementById("fechaProgramadaLabel1");
        label.innerHTML = `Programado para: Encender: ${dateTimeValue} `;
...
// Event Listener para el botón de cambiar estado del LED 1 con SweetAlert2
document.getElementById("programarBtn2luces").addEventListener("click", () => {
  Swal.fire({
    title: 'Seleccione una fecha y hora',
    html: '<input type="datetime-local" id="datetime-select" />',
    confirmButtonText: 'Aceptar',
    preConfirm: () => {
      ...
      if (dateTimeValue) {
        ...
        const label = document.getElementById("fechaProgramadaLabel1");
        label.innerHTML = `Programado para: Apagar: ${dateTimeValue} `;
...
document.getElementById("programarBtn1ventilador").addEventListener("click", () => {
  Swal.fire({
    title: 'Seleccione una fecha y hora',
    html: '<input type="datetime-local" id="datetime-select" />',
    confirmButtonText: 'Aceptar',
    preConfirm: () => {
      ...
      if (dateTimeValue) {
        ...
        const label = document.getElementById("fechaProgramadaLabel2");
        label.innerHTML = `Programado para: Encender: ${dateTimeValue} `;
...
// Event Listener para el botón de cambiar estado del LED 2 con SweetAlert2
document.getElementById("programarBtn2ventilador").addEventListener("click", () => {
  Swal.fire({
    title: 'Seleccione una fecha y hora',
    html: '<input type="datetime-local" id="datetime-select" />',
    confirmButtonText: 'Aceptar',
...
function filtrarHistorialPorFecha(fechaInicio, fechaFin) {
  const registrosList = document.getElementById("registros");
  ...
  getDoc(doc(dbHistorial, "DataPas", "Historial")).then((docSnapshot) => {
    if (docSnapshot.exists()) {
      ...
      led1.concat(led2).forEach((fecha) => {
        const fechaRegistro = new Date(fecha);
        if (fechaRegistro >= fechaInicio && fechaRegistro <= fechaFin) {
          registros.push({ tipo: led1.includes(fecha) ? 'Cambio Estado Luces' : 'Cambio Estado Ventilador', fecha });
        }
      });
...
document.getElementById("buscarPorFechaBtn").addEventListener("click", () => {
  const fechaInicio = new Date(document.getElementById("fechaInicio").value);
  const fechaFin = new Date(document.getElementById("fechaFin").value);
  filtrarHistorialPorFecha(fechaInicio, fechaFin);
});
